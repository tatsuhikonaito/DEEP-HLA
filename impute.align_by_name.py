...
def impute(args):
    ...
    concord_snp = model_bim.id.isin(sample_bim.id)
    concord_sample_snp = sample_bim.id.isin(model_bim.id)
    ...
    logger.log('{} SNPs consistent with model SNPs in name and used for imputation.'.format(np.sum(concord_snp)))
